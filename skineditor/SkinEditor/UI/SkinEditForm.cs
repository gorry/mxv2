...
public sealed class SkinEditForm : Form
{
    ...
    public SkinEditForm(SkinDocument doc)
    {
        ...
        // 2026-09-06、スピンボタンの幅を数値欄1個あたり +8px 広げた
        // （SpinWidth 参照）ぶん、矩形（4個ぶん=+32px）で再び末尾が
        // 切れたので、その分＋余裕を見て 670→720 にした。
        // 同日、Xywh の幅を「x,y は符号5桁・w,h は符号無し4桁」に統一した
        // （素材内位置のような x,y が 0 始まりの項目も x,y だけ符号付き幅に
        // 揃えた）ぶん、サブタブ内（`RowWidthFor` が rowWidth-40 を使う。
        // 入れ子 TabControl 自身の枠のぶん本来の rowWidth より狭い）の
        // 「素材内位置」で再びサフィックスが切れたので、720→760 にした。
        ...
        foreach (var section in LayoutFieldSchema.BuildSections())
        {
            ...
            FlowLayoutPanel GetSubTabFlow(string subTabTitle)
            {
                ...
                if (subTabs == null)
                {
                    ...
                    {
                        Width = rowWidth,
                        // 「ステータス」タブは一番項目数が多い「PCM」サブタブ
                        // （PCM音量/PCMポインタ + PCMの位置 + チャンネル行8つ =
                        // 11行、約408の高さ）がスクロールせずに収まる高さ
                        // （2026-09-04、ユーザー指示）。「操作ボタン」タブの
                        // サブタブは 480 だと下に大きな余白が残っていた
                        // （2026-09-06、ユーザー指摘）。一番項目数が多いのは
                        // ボタン別サブタブ（PLAY等。最大3行+押す/LEDトグル行）
                        // ではなく「パレット」サブタブ（6行、約270の高さ）。
                        // タブごとに必要な高さが大きく違うので `section.Title`
                        // で分けた（`TabControl` はページ間で高さを可変に
                        // できないため）。
                        Height = Dpi.S(this, section.Title == "操作ボタン" ? 320 : 480),
