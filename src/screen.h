...
public:
	...
	// フルスクリーン表示にしてよいか。**デスクトップだけ**（2026-09-18、
	// ユーザーの指示）。モバイルは窓＝画面なので切り替える意味が無く、
	// ブラウザ (Emscripten) は利用者の操作からでないと全画面にできない
	// 作法があるので、どちらも導線を出さない。
	static bool CanFullScreen();

	// フルスクリーン表示の切り替え。**表示モードは変えず**、窓を
	// デスクトップいっぱいに広げる (SDL_WINDOW_FULLSCREEN_DESKTOP)。
	// キャンバスは窓に追いかけて作り直される（main の SyncCanvasToWindow。
	// 窓をリサイズしたときと同じ経路を通る）。切り替えられたら true。
	bool SetFullScreen(bool on);
	// いまフルスクリーンか。SDL の窓の旗をそのまま見るので、OS 側から
	...
	// 窓の大きさをこちらから決められない状態か（フルスクリーンか最大化）。
	// **このときは表示倍率の導線を止める**——窓の大きさを OS 側が決めて
	// いるので、倍率を変えても掛けようがない（2026-09-18、ユーザーの指示）。
	// キーの Ctrl +/- (keybind.cpp) と設定ダイアログの [表示倍率]
	// (settingsui_settings.cpp) が、どちらもここを見る。
	bool windowSizeLocked() const { return fullScreen() || maximized(); }
